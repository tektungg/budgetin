            ...
            try:
                if loop.is_running():
                    future = asyncio.run_coroutine_threadsafe(
                        process_telegram_update_with_retry(update_data, attempt=1), 
                        loop
                    )
                    future.result(timeout=timeout_duration)
                else:
                    loop.run_until_complete(
                        asyncio.wait_for(process_telegram_update_with_retry(update_data, attempt=1), timeout=timeout_duration)
                    )
                success = True
            except Exception as e:
                    ...
                    try:
                        if loop.is_running():
                            retry_future = asyncio.run_coroutine_threadsafe(
                                send_retry_message(update_data), 
                                loop
                            )
                            retry_future.result(timeout=3)  # Quick 3s for retry message
                        else:
                            loop.run_until_complete(
                                asyncio.wait_for(send_retry_message(update_data), timeout=3)
                            )
                ...
                try:
                    if loop.is_running():
                        future = asyncio.run_coroutine_threadsafe(
                            process_telegram_update_with_retry(update_data, attempt=2), 
                            loop
                        )
                        future.result(timeout=timeout_duration)
                    else:
                        loop.run_until_complete(
                            asyncio.wait_for(process_telegram_update_with_retry(update_data, attempt=2), timeout=timeout_duration)
                        )
                    success = True
                    ...
                    try:
                        if loop.is_running():
                            error_future = asyncio.run_coroutine_threadsafe(
                                send_final_error_message(update_data, is_oauth=is_oauth), 
                                loop
                            )
                            error_future.result(timeout=3)  # Quick 3s for error message
                        else:
                            loop.run_until_complete(
                                asyncio.wait_for(send_final_error_message(update_data, is_oauth=is_oauth), timeout=3)
                            )
...
async def send_retry_message(update_data):
...
async def send_final_error_message(update_data, is_oauth=False):
    ...
    try:
        ...
        if update and update.effective_chat:
            if is_oauth:
                ...
                text = ("❌ *Operasi gagal setelah 2 percobaan (masing-masing 5 detik)*\n\n"
                       "🔧 *Yang bisa Anda lakukan:*\n"
...
async def process_telegram_update_with_retry(update_data, attempt=1):
